...
class Centurio
{
    ...
    function getProductsbyShopAndBrand($DB,$siteId,$brandId){
        return ($DB->query("SELECT * FROM results JOIN sku ON results.pos_id = sku.id JOIN brands ON brands.id = sku.brand_id WHERE brand_id =$brandId and site_id= 5", PDO::FETCH_ASSOC)->fetchAll());
    }
    ...
    function publicResult($DB, $siteId, $brandId)
    {
        ...
        $arr =  ($DB->query("SELECT * FROM results LEFT JOIN sites ON results.site_id = sites.id JOIN sku ON results.pos_id = sku.id JOIN brands ON sku.brand_id = brands.id WHERE sites.id = $siteId and brands.id =2", PDO::FETCH_ASSOC)->fetchAll());
        $cnt = count($arr);
        if(!$cnt){
